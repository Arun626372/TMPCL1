(function(){
  ...
  function preferredPublicImage(urlValue='', uploadedValue='', oldValue=''){
    const url = cleanUrl(urlValue);
    return url || uploadedValue || oldValue || '';
  }
  ...
  async function renderDashboard(){
    ...
    try{
      ...
      const pTb=$('#partnersAdminTable tbody'); if(pTb){ $('#emptyPartnersAdmin').style.display=partners.length?'none':'block'; pTb.innerHTML=partners.map(p=>{ const logo=p.logo_url?`<img loading="lazy" src="${esc(p.logo_url)}" style="width:44px;height:44px;border-radius:12px;object-fit:cover">`:`<strong>${initials(p.name,'TP')}</strong>`; return `<tr><td>${logo}</td><td>${esc(p.name)}</td><td>${esc(p.category||'')}</td><td>${esc(p.status||'')}</td><td>${p.link?`<a href="${esc(p.link)}" target="_blank">Open</a>`:''}</td><td>${esc(p.description||'')}</td><td><div class="admin-table-actions"><button class="mini-btn" data-edit-partner="${esc(p.id)}">Edit</button><button class="mini-btn danger" data-delete-partner="${esc(p.id)}">Delete</button></div></td></tr>`; }).join('');
        $$('[data-edit-partner]',pTb).forEach(btn=>btn.addEventListener('click',()=>{ const p=partners.find(x=>x.id===btn.dataset.editPartner); if(!p) return; $('#partnerId').value=p.id; $('#partnerName').value=p.name; $('#partnerCategory').value=p.category||'Title Sponsor'; $('#partnerLink').value=p.link||''; $('#partnerDescription').value=p.description||''; $('#partnerStatus').value=p.status||'Published'; if($('#partnerLogoUrl')) $('#partnerLogoUrl').value=p.logo_url||''; showSuccess($('#partnerManageSuccess'), '<strong>Edit mode:</strong> Change fields and click Publish / Update Partner.'); document.querySelector('.admin-tab-panel.active')?.scrollIntoView({behavior:'smooth',block:'start'}); }));
      ...
      const newsTb=$('#newsAdminTable tbody'); if(newsTb){ $('#emptyNewsAdmin').style.display=news.length?'none':'block'; newsTb.innerHTML=news.map(n=>{ const img=n.image_url?`<img loading="lazy" src="${esc(n.image_url)}" style="width:58px;height:42px;border-radius:8px;object-fit:cover">`:''; return `<tr><td>${img}</td><td>${esc(n.title)}</td><td>${esc(n.tag||'')}</td><td>${esc(n.date||'')}</td><td>${esc(n.summary||'')}</td><td><div class="admin-table-actions"><button class="mini-btn" data-edit-news="${esc(n.id)}">Edit</button><button class="mini-btn danger" data-delete-news="${esc(n.id)}">Delete</button></div></td></tr>`; }).join('');
        $$('[data-edit-news]',newsTb).forEach(btn=>btn.addEventListener('click',()=>{ const n=news.find(x=>x.id===btn.dataset.editNews); if(!n) return; $('#newsId').value=n.id; $('#newsTitle').value=n.title; $('#newsTag').value=n.tag||'Announcement'; $('#newsDate').value=n.date||''; $('#newsSummary').value=n.summary||''; if($('#newsImageUrl')) $('#newsImageUrl').value=n.image_url||''; showSuccess($('#newsSuccess'), '<strong>Edit mode:</strong> Update fields and save.'); document.querySelector('.admin-tab-panel.active')?.scrollIntoView({behavior:'smooth',block:'start'}); }));
      ...
      if(regTb){
        ...
        const drawRegs = () => {
          ...
          regTb.innerHTML=list.map(r=>`<tr><td>${esc(r.id)}</td><td>${esc(r.name||'')}</td><td>${esc(r.mobile||'')}</td><td><strong>${esc(r.aadhaar_last4 || r.aadhaarLast4 || '-')}</strong></td><td>${(r.document_consent === true || r.documentConsent === true) ? 'Yes' : 'No'}</td><td>${esc(r.city||'')}</td><td>${esc(r.trial_location||'')}</td><td>${esc(r.dob||'')}</td><td>${esc(r.age||'')}</td><td>${esc(r.role||'')}</td><td>${esc(r.age_group||'')}</td><td>${esc(r.payment_status||'Pending')}</td><td>${esc(r.assigned_category||'Pending until trials & selection')}</td><td>${r.photo_url?`<a href="${esc(r.photo_url)}" target="_blank">View</a>`:''}</td><td>${r.proof_url?`<a href="${esc(r.proof_url)}" target="_blank">View</a>`:''}</td><td>₹${esc(r.fee||'999')}</td><td>${esc(formatDateSafe(r.created_at)||'')}</td></tr>`).join('');
        };
  ...
  function setupDashboardForms(){
    ...
    if(teamForm){ teamForm.addEventListener('submit', async e=>{ e.preventDefault(); const btn=teamForm.querySelector('button[type="submit"]'); setLoading(btn,true); try{ const fd=new FormData(teamForm); const id=fd.get('teamId')||('team-'+Date.now()); const old=(await selectRows('teams',`select=*&id=eq.${encodeURIComponent(id)}&limit=1`))[0]||{}; const uploadedLogo=await uploadFile(STORAGE_BUCKETS.teamLogo,$('#teamLogo'),id); const uploadedBanner=await uploadFile(STORAGE_BUCKETS.squadBanner,$('#squadBanner'),id); const logo=preferredPublicImage(fd.get('teamLogoUrl'), uploadedLogo, old.logo_url); const banner=preferredPublicImage(fd.get('squadBannerUrl'), uploadedBanner, old.squad_banner_url); await saveRow('teams',{id,name:fd.get('teamName'),city:fd.get('teamCity'),owner:fd.get('teamOwner')||'',status:fd.get('teamStatus')||'Team Selection Pending',logo_url:logo,squad_banner_url:banner}); showSuccess($('#teamSuccess'), '<strong>Team saved successfully.</strong> Public Teams page par update show hoga.'); teamForm.reset(); $('#teamId').value=''; renderDashboard(); } catch(err){ showDbError('Team save',err); } finally{ setLoading(btn,false); } }); $('#teamFormReset')?.addEventListener('click',()=>{teamForm.reset(); $('#teamId').value=''; $('#teamSuccess').style.display='none';}); }

    const partnerManageForm=$('#partnerManageForm');
    if(partnerManageForm){ partnerManageForm.addEventListener('submit', async e=>{ e.preventDefault(); const btn=partnerManageForm.querySelector('button[type="submit"]'); setLoading(btn,true); try{ const fd=new FormData(partnerManageForm); const id=fd.get('partnerId')||('partner-'+Date.now()); const old=(await selectRows('partners',`select=*&id=eq.${encodeURIComponent(id)}&limit=1`))[0]||{}; const uploadedLogo=await uploadFile(STORAGE_BUCKETS.partnerLogo,$('#partnerLogo'),id); const logo=preferredPublicImage(fd.get('partnerLogoUrl'), uploadedLogo, old.logo_url); await saveRow('partners',{id,name:fd.get('partnerName'),category:fd.get('partnerCategory')||'Partner',link:fd.get('partnerLink')||'',description:fd.get('partnerDescription')||'',status:fd.get('partnerStatus')||'Published',logo_url:logo}); showSuccess($('#partnerManageSuccess'), '<strong>Partner saved successfully.</strong>'); partnerManageForm.reset(); $('#partnerId').value=''; renderDashboard(); } catch(err){ showDbError('Partner save',err); } finally{ setLoading(btn,false); } }); $('#partnerFormReset')?.addEventListener('click',()=>{partnerManageForm.reset(); $('#partnerId').value=''; $('#partnerManageSuccess').style.display='none';}); }

    const galleryForm=$('#galleryForm');
    if(galleryForm){ galleryForm.addEventListener('submit', async e=>{ e.preventDefault(); const btn=galleryForm.querySelector('button[type="submit"]'); setLoading(btn,true); try{ const fd=new FormData(galleryForm); const id=fd.get('mediaId')||('media-'+Date.now()); const old=(await selectRows('gallery_media',`select=*&id=eq.${encodeURIComponent(id)}&limit=1`))[0]||{}; const uploadedImage=await uploadFile(STORAGE_BUCKETS.gallery,$('#mediaImage'),id); const image=preferredPublicImage(fd.get('mediaImageUrl'), uploadedImage, old.image_url); await saveRow('gallery_media',{id,title:fd.get('mediaTitle'),type:fd.get('mediaType')||'photos',category:fd.get('mediaCategory')||'events',date:fd.get('mediaDate')||null,url:fd.get('mediaUrl')||'',image_url:image}); showSuccess($('#gallerySuccess'), '<strong>Gallery media published.</strong>'); galleryForm.reset(); $('#mediaId').value=''; renderDashboard(); } catch(err){ showDbError('Gallery save',err); } finally{ setLoading(btn,false); } }); $('#galleryFormReset')?.addEventListener('click',()=>{galleryForm.reset(); $('#mediaId').value=''; $('#gallerySuccess').style.display='none';}); }

    const newsForm=$('#newsForm');
    if(newsForm){ newsForm.addEventListener('submit', async e=>{ e.preventDefault(); const btn=newsForm.querySelector('button[type="submit"]'); setLoading(btn,true); try{ const fd=new FormData(newsForm); const id=fd.get('newsId')||('news-'+Date.now()); const old=(await selectRows('news_updates',`select=*&id=eq.${encodeURIComponent(id)}&limit=1`))[0]||{}; const uploadedImage=await uploadFile(STORAGE_BUCKETS.newsImage,$('#newsImage'),id); const image=preferredPublicImage(fd.get('newsImageUrl'), uploadedImage, old.image_url); await saveRow('news_updates',{id,title:fd.get('newsTitle'),tag:fd.get('newsTag')||'Announcement',date:fd.get('newsDate')||null,summary:fd.get('newsSummary')||'',image_url:image}); showSuccess($('#newsSuccess'), '<strong>News published.</strong>'); newsForm.reset(); $('#newsId').value=''; renderDashboard(); } catch(err){ showDbError('News save',err); } finally{ setLoading(btn,false); } }); $('#newsFormReset')?.addEventListener('click',()=>{newsForm.reset(); $('#newsId').value=''; $('#newsSuccess').style.display='none';}); }

    const leadershipForm=$('#leadershipForm');
    if(leadershipForm){ leadershipForm.addEventListener('submit', async e=>{ e.preventDefault(); const btn=leadershipForm.querySelector('button[type="submit"]'); setLoading(btn,true); try{ const fd=new FormData(leadershipForm); const id=fd.get('personId')||('leader-'+Date.now()); const old=(await selectRows('leadership_panel',`select=*&id=eq.${encodeURIComponent(id)}&limit=1`))[0]||{}; const uploadedPhoto=await uploadFile(STORAGE_BUCKETS.leadershipPhoto,$('#personPhoto'),id); const photo=preferredPublicImage(fd.get('personPhotoUrl'), uploadedPhoto, old.photo_url); await saveRow('leadership_panel',{id,name:fd.get('personName'),type:fd.get('personType')||'Selector',designation:fd.get('personDesignation')||fd.get('personType'),bio:fd.get('personBio')||'',photo_url:photo}); showSuccess($('#leadershipSuccess'), '<strong>Profile saved successfully.</strong>'); leadershipForm.reset(); $('#personId').value=''; renderDashboard(); } catch(err){ showDbError('Leadership save',err); } finally{ setLoading(btn,false); } }); $('#leadershipFormReset')?.addEventListener('click',()=>{leadershipForm.reset(); $('#personId').value=''; $('#leadershipSuccess').style.display='none';}); }
  }
