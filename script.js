(function(){
  ...
  async function renderPublicTeams(){
    ...
    try{
      ...
      }).join('') || `<article class="card"><h3>Teams Coming Soon</h3><p class="muted">Official team details will be announced soon.</p></article>`;
  ...
  async function renderHomePreviews(){
    ...
    try{
      ...
      if(galleryEl){
        ...
        galleryEl.textContent = media[0]?.title || 'Photos, videos and event moments';
      }
      if(partnersEl){
  ...
  async function renderPublicLeadership(){
    ...
    try{
      ...
      if(amb){ amb.innerHTML = ambassadors.length ? ambassadors.slice(0,2).map(ambassadorCard).join('') : `<article class="card empty-state"><h3>Brand Ambassadors Coming Soon</h3><p class="muted">Official brand ambassador details will be announced soon.</p></article>`; bindProfileCards(amb); }
