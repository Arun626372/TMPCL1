(function(){
  ...
  async function renderPublicTeams(){
    ...
    try{
      ...
      }).join('') || `<article class="card"><h3>Teams Coming Soon</h3><p class="muted">Teams publish hone ke baad yahan show hongi.</p></article>`;
  ...
  async function renderPublicLeadership(){
    ...
    try{
      ...
      if(amb){ amb.innerHTML = ambassadors.length ? ambassadors.slice(0,2).map(ambassadorCard).join('') : `<article class="card empty-state"><h3>Brand Ambassadors Coming Soon</h3><p class="muted">Brand ambassador profiles publish hone ke baad yahan show honge.</p></article>`; bindProfileCards(amb); }
